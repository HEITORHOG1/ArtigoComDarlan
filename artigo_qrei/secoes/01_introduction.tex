...
This paper introduces the \emph{Quantitative-Coupled BowTie}
(QC-BowTie), a probabilistic barrier-screening method for design-stage
reliability, and demonstrates it on the valve
train of a high-performance cylinder head operating at 10\,000~rpm in the
VW AP 1.8 engine. \textbf{The deliverable of this paper is the method;
the AP 1.8 valve train is the demonstrator on which it is exercised, not
the object of a design verdict.} The qualitative bow diagram is enriched with a
...
sits between the reliability lessons of published valve-train
failure studies and design-stage prevention: it takes the root causes reported in the
...
\emph{reliability-informed, design-stage prevention}: the
failure-mechanism evidence is borrowed faithfully from
peer-reviewed case studies; the original step is the
translation of those root causes into a reproducible,
indicator-driven barrier check applicable at the design stage.
Quantitative extensions of the BowTie diagram exist
(\citealp{khakzad2013bowtiebayesian}, via Bayesian-network mapping;
LOPA, fault-tree-coupled BowTie, and DyPASI in process safety), but they
quantify barrier \emph{failure frequencies} from event or failure-rate
data. The QC-BowTie differs in kind: it binds each preventive barrier to
a \emph{deterministic engineering indicator} computed from
first-principles sizing, and then propagates that indicator through its
manufacturing tolerance to a probability of compliance failure. Unlike
LOPA, which assumes independent protection layers with catalogued
probabilities of failure on demand, the QC-BowTie derives each
barrier's status from the physics of the component itself, and therefore
applies at the design stage \emph{before} any failure-rate data exist.
Two original contributions follow: (i) the barrier-to-indicator
coupling that turns each bow-tie barrier into an auditable pass/fail
check; and (ii) the mapping of the Monte Carlo compliance-failure
probability onto the BowTie barrier-status language
(\emph{active}/\emph{marginal}/\emph{at-risk}), which surfaces a
\emph{marginal} inner-spring barrier
(Section~\ref{sec:res-mc}) that the deterministic layer classifies as
safe, the clearest demonstration that the probabilistic layer sees
what a nominal-point analysis does not. The procedure is
...
for the AP 1.8 case, and the construct-validity check of the procedure
